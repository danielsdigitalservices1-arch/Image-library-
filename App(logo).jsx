...
export default function App() {
  ...
  const [testTitle] = useState(() => localStorage.getItem('cbt_testTitle') || "ANA 213: GENERAL EMBRYO AND GENETICS");
  ...
  const LOGO_URL = "https://raw.githubusercontent.com/danielsdigitalservices1-arch/Image-library-/e3e6abfd70dbb651b780cd8a841d5dea4beca979/logo.png"; // Paste your actual link here
  const [logoError, setLogoError] = useState(false);

  // --- 2. QUESTIONS STATE ---
  // MODIFIED: Start with empty array, do not load from localStorage
  ...
  // --- EFFECT: LOAD QUESTIONS FROM SUPABASE ON MOUNT ---
  useEffect(() => {
    const fetchQuestions = async () => {
      setIsLoadingQuestions(true);
      try {
        const { data, error } = await questionSupabase
          .from('questions')
        ...
        if (data && data.length > 0) {
          // 1. Format questions (Same as your existing code)
          let formattedQuestions = data.map((item, idx) => {
            const clean = (str) => str?.toString().trim() || '';
            let correctKey = clean(item.correct_answer).toLowerCase();
          ...
          // 2. SECURE FIX: Re-sort questions if a saved order exists
          const savedOrder = localStorage.getItem('cbt_question_order');
          ...
          if (savedOrder) {
            try {
               ...
               // Rebuild the array in the saved order
               const reorderedQuestions = orderList
                 .map(id => questionMap.get(id))
                 .filter(q => q !== undefined); // Remove any IDs that might not exist anymore
               
               // Only use reordered if lengths match (safety check)
               if (reorderedQuestions.length > 0) {
                   formattedQuestions = reorderedQuestions;
               }
            } catch (e) {
  ...
  const startTest = () => {
    if (questions.length === 0) {
      alert("Questions are still loading or failed to load. Please refresh.");
      return;
    }
    ...
    // 2. SECURE FIX: Save ONLY the order of IDs to localStorage
    // This way, no answers are stored, just a list like ["id-1", "id-5", "id-2"]
    const idList = randomized.map(q => q.id);
    localStorage.setItem('cbt_question_order', JSON.stringify(idList));

    // 3. Set timer and view
  ...
  const handleSubmit = async () => {
    ...
    // Grab the absolute latest data from the refs
    const latestAnswers = answersRef.current;
    const latestQuestions = questionsRef.current;

    try {
      // Send the answers to our secure Vercel API backend
      const response = await fetch('/api/grade', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          answers: latestAnswers, // <-- Use latestAnswers here
          guestId,
          appName,
          testTitle,
          marksPerQuestion,
      ...
      // Inside handleSubmit in App.jsx
      const resultData = {
        score: serverResult.score,
        total: serverResult.total,
        percentage: serverResult.percentage,
        statusTier: serverResult.statusTier, // <-- Add this line
      ...
      // ... (The rest of your handleSubmit code remains exactly the same from sessionStorage.setItem downward)

      sessionStorage.setItem('cbt_currentResult', JSON.stringify(resultData));

      // Only clear storage and change view IF successful
      localStorage.removeItem('cbt_endTime');
      localStorage.removeItem('cbt_answers');
      localStorage.removeItem('cbt_currentIndex');
      localStorage.removeItem('cbt_question_order'); 
      setView('result');
...
const handleExitToWelcome = () => {
    ...
    localStorage.removeItem('cbt_answers');
    localStorage.removeItem('cbt_currentIndex');
    localStorage.removeItem('cbt_question_order'); // <--- Add this
    sessionStorage.removeItem('cbt_currentResult');
    setView('welcome');
};
      ...
      <div className="fixed inset-0 bg-gray-50 flex items-center justify-center p-4 font-sans text-gray-900 overflow-hidden">
        ...
        <div className="bg-white p-5 md:p-10 rounded-2xl shadow-xl max-w-3xl w-full text-center border border-blue-100 relative flex flex-col justify-center shrink-0 max-h-full overflow-y-auto">
          ...
          <div className="mb-4 md:mb-6 flex flex-col items-center justify-center gap-3">
             {!logoError ? (
               <img 
                 ...
                 className="w-24 h-24 md:w-32 md:h-32 rounded-full object-cover border border-gray-100 shadow-sm"
               />
             ) : (
          ...
          <div className="text-left bg-gray-50 p-4 rounded-lg text-sm text-gray-700 mb-4 md:mb-8 space-y-2">
            ...
            <p className="flex items-center"><FileText className="w-4 h-4 mr-2"/> <strong>Questions:</strong> {isLoadingQuestions ? 'Loading...' : questions.length}</p>
          ...
          <button 
            ...
            className={`
              w-full text-white font-bold py-3 md:py-4 rounded-xl shadow-lg transition transform flex items-center justify-center mb-4 md:mb-6 flex-shrink-0
          ...
          >
             {isLoadingQuestions ? (
               <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Loading Questions...</>
             ) : (
  ...
  // ---------------- RENDER: TEST ----------------
  if (view === 'test') {
    // Safety check in case questions aren't loaded or index is out of bounds
    ...
    if(!question) return null; // Should not happen if lengths align
        ...
        <header className="flex-shrink-0 z-50 bg-white/90 backdrop-blur-md shadow-sm px-3 md:px-6 py-3 flex justify-between items-center w-full relative transition-all">
          <div className="flex flex-1 items-center gap-2 overflow-hidden mr-2 min-w-0">
            ...
            >
             ...
             {/* LOGO (Visible only on laptop/desktop) */}
            {!logoError && (
              <img 
                src={LOGO_URL} 
                alt="App Logo"
                onError={() => setLogoError(true)}
                className="hidden md:block w-10 h-10 md:w-16 md:h-16 rounded-full object-cover border border-gray-100 shadow-sm flex-shrink-0 mr-1 md:mr-1"
              />
            )}
            ...
            <div className="flex flex-col overflow-hidden min-w-0 items-start flex-1 w-full">
               ...
               <h1 className="font-bold text-blue-600 text-xs md:text-lg uppercase truncate w-full block leading-tight mb-1">{testTitle}</h1>
               <GuestIdBadge id={guestId} />
            </div>
          </div>
  ...
  if (view === 'result') {
    ...
    // Inside the view === 'result' block in App.jsx
    const resultData = JSON.parse(resultDataStr);
    const pct = resultData.percentage;
    const statusTier = resultData.statusTier; // <-- Read directly from your data

    // Only map the visual elements (colors, icons, messages) based on the tier
    ...
    return (
      <div className="fixed inset-0 bg-gray-50 flex flex-col font-sans text-gray-900 overflow-hidden">
        ...
        <div className="flex-shrink-0 z-50 bg-white/90 backdrop-blur-md shadow-sm border-b border-gray-100 px-4 py-3 flex items-center justify-between">
           ...
           {/* UPDATE THE CENTERED CONTAINER IN YOUR RESULT HEADER TO LOOK LIKE THIS: */}
